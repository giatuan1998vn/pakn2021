...
class _timKiemState extends State<timKiem> {
  ...
  Future<Null> _selectDateTN(BuildContext context) async {
    ...
    if (picked != null)
      if(mounted){setState(() {
        selectedDate = picked;
        _dateControllerTN.text =  DateFormat('yyyy-MM-dd')
            .format(DateFormat('yyyy-MM-dd').parse(selectedDate.toString()));
        String tn =DateFormat('dd-MM-yyyy')
            .format(DateFormat('yyyy-MM-dd').parse(selectedDate.toString()));
        GetDataKeyWord("",tn,"","");
      });}
  ...
  Future<Null> _selectDateDN(BuildContext context) async {
    ...
    if (picked != null)
      if(mounted){ setState(() {
        selectedDate = picked;
        _dateControllerDN.text =  DateFormat('yyyy-MM-dd')
            .format(DateFormat('yyyy-MM-dd').parse(selectedDate.toString()));
        String dn =DateFormat('dd-MM-yyyy')
            .format(DateFormat('yyyy-MM-dd').parse(selectedDate.toString()));
        GetDataKeyWord("","",dn,"");
      });}
  ...
  GetDataKeyWord(String title,String tungay,String denngay,String cq) async {
    ...
    return Scaffold(
        ...
        appBar:   AppBar(
          ...
          leading: new IconButton(
            ...
            preferredSize: Size.fromHeight(kToolbarHeight+170),
          child: Container(
